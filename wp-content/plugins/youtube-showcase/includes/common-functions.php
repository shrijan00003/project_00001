...
if (!function_exists('emd_is_visible')) {
	function emd_is_item_visible($item,$app,$type,$check_full=0) {
		...
		elseif($type == 'relation'){
			...
			if(!empty($ent_map_list[$post->post_type])){
				if((empty($ent_map_list[$post->post_type]['hide_rels'])) || (!empty($ent_map_list[$post->post_type]['hide_rels'][$item]) && $ent_map_list[$post->post_type]['hide_rels'][$item] == 'show')){
					$visible = 1;
				}
			}
			if(!empty($ent_map_list[$other_ptype])){
			...
			if(empty($ent_map_list[$post->post_type]['hide_rels']) && empty($ent_map_list[$other_ptype]['hide_rels'])) {
				$visible = 1;
			}
			if($visible == 1 && (!empty($ent_map_list[$post->post_type]['edit_rels'][$myrole][$item]) && $ent_map_list[$post->post_type]['edit_rels'][$myrole][$item] == 'not_show')){
				$visible = 0;
			}
...
if(!function_exists('emd_get_attachment_layout')){
	function emd_get_attachment_layout($info){
		...
		echo "<div class='att-file' style='padding:5px;display:inline-block;'>" . $fsrc . "</div>";
	}
}
