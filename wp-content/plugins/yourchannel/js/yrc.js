...
jQuery(document).ready(function($){
	...
	YRC.Base.prototype = {		
		...
		'list': function(){			
			...
			if( bl.length ){
				this.temp_items = this.temp_items.filter(function(item){
					return !( item.snippet.title === 'Private video' || item.snippet.title === 'Deleted video' || bl.indexOf(item.id) > -1 );
				});
			}
		...
		sortList: function(){
			...
			if((srt !== 'none') && (( this.onlyonce || this.data.meta.playlist || this.data.meta.custom) || srt === 'title_desc') ){
		...
		'listVideos': function(cont, res){
			...
			if( (this.request.times === 1 || this.multi_page) &&  this.theme.style === '__sidebar' ){
				core.find('a.yrc-video-link').eq(0).trigger('click');
			}
		},
