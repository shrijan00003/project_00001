...
if ( ! class_exists( 'Widget_Nepali_Date_Converter' ) ) {
    /**
     ...
     * @since 1.0
     */
    class Widget_Nepali_Date_Converter extends WP_Widget {
        function __construct() {
            ...
            if(is_active_widget(false, false, $this->id_base)) {
                add_action( 'wp_enqueue_scripts', 'nepali_date_converter_enqueue_scripts' );
                add_action('wp_footer', 'nepali_date_converter_wp_footer');
            }
        }
        /*Widget Backend*/
        public function form( $instance ) {
            ...
            if ( isset( $instance[ 'disable_convert_nep_to_eng' ] ) ) {
                $disable_convert_nep_to_eng = $instance[ 'disable_convert_nep_to_eng' ];
            }
            else {
                $disable_convert_nep_to_eng = '';
            }

            if ( isset( $instance[ 'disable_convert_eng_to_nep' ] ) ) {
                $disable_convert_eng_to_nep = $instance[ 'disable_convert_eng_to_nep' ];
            }
            else {
            ...
            <p>
                <input id="<?php echo $this->get_field_id('disable_convert_nep_to_eng'); ?>" name="<?php echo $this->get_field_name('disable_convert_nep_to_eng'); ?>" type="checkbox" value="1" <?php checked( 1, $disable_convert_nep_to_eng ); ?> />
                <label for="<?php echo $this->get_field_id('disable_convert_nep_to_eng'); ?>"><?php _e('Disable convert nep to eng', 'nepali-date-converter'); ?></label><br>

                <input id="<?php echo $this->get_field_id('disable_convert_eng_to_nep'); ?>" name="<?php echo $this->get_field_name('disable_convert_eng_to_nep'); ?>" type="checkbox" value="1" <?php checked( '1', $disable_convert_eng_to_nep ); ?> />
                <label for="<?php echo $this->get_field_id('disable_convert_eng_to_nep'); ?>"><?php _e('Disable convert eng to nep', 'nepali-date-converter'); ?></label><br>
            </p>
            <p>
        ...
        public function update( $new_instance, $old_instance ) {
            ...
            $instance['disable_convert_nep_to_eng'] = ( ! empty( $new_instance['disable_convert_nep_to_eng'] ) ) ? strip_tags( $new_instance['disable_convert_nep_to_eng'] ) : '';
            $instance['disable_convert_eng_to_nep'] = ( ! empty( $new_instance['disable_convert_eng_to_nep'] ) ) ? strip_tags( $new_instance['disable_convert_eng_to_nep'] ) : '';
    ...
    } // Class Widget_Nepali_Date_Converter ends here
...
if ( ! function_exists( 'coder_widget_nepali_date_converter' ) ) :
    ...
    function coder_widget_nepali_date_converter() {
        register_widget( 'Widget_Nepali_Date_Converter' );
    }
endif;
